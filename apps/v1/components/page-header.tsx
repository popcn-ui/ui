...
function PageHeader({ className, children, ...props }: PageHeaderProps) {
  return (
    <section
      className={cn("mx-auto flex max-w-4xl flex-col items-center gap-4 text-center", className)}
      {...props}
    >
      {children}
    </section>
...
function PageHeaderDescription({ className, children, ...props }: PageHeaderDescriptionProps) {
  return (
    <p className={cn("text-muted-foreground max-w-2xl text-lg sm:text-xl", className)} {...props}>
      {children}
    </p>
  )
}
